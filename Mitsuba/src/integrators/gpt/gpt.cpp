...
static const size_t VARIANCE_TP = 5;  ///< Buffer index for the noisy color image variance.
static const size_t VARIANCE_DX = 6;  ///< Buffer index for the X gradients variance.
static const size_t VARIANCE_DY = 7;  ///< Buffer index for the Y gradients variance.
static const size_t BUFFER_DEBUG = 8;
...
public:
	...
	void evaluatePoint(RadianceQueryRecord& rRec, const Point2& samplePosition, const Point2& apertureSample, Float timeSample, Float differentialScaleFactor,
		Spectrum& out_very_direct, Spectrum& out_throughput, Spectrum *out_gradients, Spectrum *out_neighborThroughputs, Float *alphas, Spectrum *out_debug, bool compute_shifts)
	{
		...
		evaluate(mainRay, shiftedRays, 4, very_direct, alphas, out_debug, compute_shifts);
	...
	void evaluate(RayState& main, RayState* shiftedRays, int secondaryCount, Spectrum& out_veryDirect, Float *alphas, Spectrum *out_debug, bool compute_shifts) {
		...
		Spectrum mainVeryDirect((Float)0);
		...
		//Marco: booleans used to visualize surface classification.
		bool usedHalfVectorShift = false;
		bool usedHalfVectorShiftDirect = false;
				...
				if (compute_shifts){
					...
					if (shifted.alive) {
						...
						if (shifted.connection_status == RAY_CONNECTED) {
							...
							if (main.rRec.depth == 1 && mainVertexType != VERTEX_TYPE_DIFFUSE)
								usedHalfVectorShiftDirect = true;
	...
	const bool &stop, const std::vector< TPoint2<uint8_t> > &points) const
{
	...
	for (size_t i = 0; i < points.size(); ++i) {
		...
		//helper for variance computation!
		std::vector<Spectrum> v_mean(5, Spectrum(0.f));
		std::vector<Spectrum> v_meanSqr(5, Spectrum(0.f));
		std::vector<Spectrum> jdet_mean(5, Spectrum(0.f));
		std::vector<Spectrum> jdet_meanSqr(5, Spectrum(0.f));
		int currentSample = 0;
		...
		for (size_t j = 0; j < sampleCount/*sampler->getSampleCount()*/; ++j) {
			...
			tracer.evaluatePoint(rRec, mv_samplePos, apertureSample, timeSample, diffScaleFactor, centralVeryDirect, centralThroughput, gradients, shiftedThroughputs, alphas, NULL, !m_config.m_disableGradients);
		...
		if (currentSample > 2){
			...
			float weight = currentSample*currentSample / (currentSample - 1); //division by (currentSample - 1) is to get the sampleVariance (see Knuth algorithm) 
			block->put(centralPixel, weight*v_meanSqr[0], 1.0f, 1.0f, VARIANCE_TP);
		}
		...
		if (!m_config.m_disableGradients){
			...
			//float weights[5] = { m_config.m_reconstructAlpha, 0.25, 0.25, 0.25, 0.25 }; //hardcoded, alpha^2=0.2^2, rest 1^2 (should rest not be 0.5^2 ? )
			...
			if (currentSample > 2){
				//weight is for mixing variance from passes. Sine we add only 1 value per pass mitsuba will combine them as (pass1 + pass2)/2
				...
				block->put(leftPixel,		(weight*0.25f)*v_meanSqr[1 + LEFT],		1.0f, 1.0f, VARIANCE_DX);
				block->put(centralPixel,	(weight*0.25f)*v_meanSqr[1 + RIGHT],	1.0f, 1.0f, VARIANCE_DX);
				block->put(topPixel,		(weight*0.25f)*v_meanSqr[1 + TOP],		1.0f, 1.0f, VARIANCE_DY);
				block->put(centralPixel,	(weight*0.25f)*v_meanSqr[1 + BOTTOM],	1.0f, 1.0f, VARIANCE_DY);
			}
			//	block->put(Point2(offset.x, offset.y), variance, 1.0f, 1.0f, VARIANCE);
	...
	int sceneResID, int sensorResID, int samplerResID)
{
	...
	outWrite ={ (m_config.m_isBase && m_config.m_useMotionVectors), 1, 1, 1, 1,                                  // motion, primal, dx, dy, direct
				0, 0, 0,                                                                                          // do not write variances to disk; var_primal, var_dx, var_dy
	...
	for (int sampling_iter=0; sampling_iter < n_iter; sampling_iter++){
		...
		if (m_config.m_useAdaptive && m_config.m_isBase && sampling_iter < n_iter - 1)
		{
			...
			film->developMulti(Point2i(0, 0), film->getCropSize(), Point2i(0, 0), dxBitmap, BUFFER_DX);
			film->developMulti(Point2i(0, 0), film->getCropSize(), Point2i(0, 0), dyBitmap, BUFFER_DY);
			film->developMulti(Point2i(0, 0), film->getCropSize(), Point2i(0, 0), varianceBitmap, VARIANCE_TP);
			film->developMulti(Point2i(0, 0), film->getCropSize(), Point2i(0, 0), varianceDxBitmap, VARIANCE_DX);
			film->developMulti(Point2i(0, 0), film->getCropSize(), Point2i(0, 0), varianceDyBitmap, VARIANCE_DY);
			if (sampleCount >= 2)
